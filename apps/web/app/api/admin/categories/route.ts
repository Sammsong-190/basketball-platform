...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: '分类 ID 为必填项' }, { status: 400 })
    }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (category.children.length > 0) {
      return NextResponse.json({ error: '无法删除：该分类下仍有子分类' }, { status: 400 })
    }

    if (category.products.length > 0) {
      return NextResponse.json({ error: '无法删除：分类下仍有商品' }, { status: 400 })
    }
